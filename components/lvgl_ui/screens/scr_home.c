...
static void update_timer_cb(lv_timer_t *t)
{
    ...
    /* Rango aproximado: 3.0V = 0%, 4.2V = 100% */
...
{
    /* Inicializar sensor de temperatura una sola vez */
...
void scr_home_init(void)
{
    ...
    lv_obj_set_style_bg_color(ui_scrHome, lv_color_black(), 0);
    ...
    lv_obj_set_size(arc_battery, 120, 120);
    ...
    lv_obj_align(lbl_uptime, LV_ALIGN_CENTER, 0, 60);

    /* Timer de actualización cada 1 segundo */
    timer_update = lv_timer_create(update_timer_cb, 1000, NULL);
    lv_timer_ready(timer_update); /* dispara inmediatamente */
}
